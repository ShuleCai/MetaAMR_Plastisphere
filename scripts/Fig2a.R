pkgs <- c("dplyr", "patchwork", "ggplot2")
for (p in pkgs) {
...
plot_func <- function(split_name) {
  effect_df <- data_df %>% filter(split == split_name) %>% mutate(var = factor(var, levels = var_levels),
                                             index = factor(index, levels = c("ACC", "AMCC"))) %>% arrange(var, index) %>%
    mutate(x = paste0(var, "_", index)) %>% mutate(x = factor(x, levels = rev(x)))
  p <- ggplot(effect_df, aes(x, estimate)) +
    annotate('rect',
             xmin = seq(0.5, n_distinct(effect_df$x)-0.5, by = 4),
             xmax = seq(2.5, n_distinct(effect_df$x)+0.5, by = 4),
...
p_combined <- plot_func("All") + plot_func("Freshwater") + plot_func("Marine")

ggsave(p_combined, filename = "./figures/Fig2a.pdf", width = 15, height = 7)

...
data_df <- read.csv("./data/Fig6b.csv")
plot_data <- data_df %>% 
  mutate(group = factor(group, levels = c("plas", "env"))) 

label_y <- max(plot_data[["Genome_Size"]]) * 0.98

# Violin plot
ggplot(plot_data, aes(x = group, y = .data[["Genome_Size"]], fill = group)) +
...
ggsave("./figures/Fig6b.pdf", width = 6, height = 6)

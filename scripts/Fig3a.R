...
data_df <- read.csv("./data/Fig3a.csv")
plot_func <- function(split_name) {
  effect_df <- data_df %>% filter(split == split_name) %>% mutate(x = factor(var, levels = data_df$var %>% unique %>% rev))
...
ggsave(p_combined, filename = "./figures/Fig3a.pdf", width = 14, height = 6)

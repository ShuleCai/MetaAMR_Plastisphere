...
data_df <- read.csv("./data/Fig2b.csv")
var_levels <- c("Phage", "IS", "CE", "IGE", "Plasmid")  

plot_func <- function(split_name) {
  effect_df <- data_df %>% filter(split == split_name) %>% mutate(x = factor(var, levels = rev(var_levels)))
...
ggsave(p_combined, filename = "./figures/Fig2b.pdf", width = 14, height = 6)
